...
void RenderManager::draw_texture(const mat3& projection, vec2 position, float rotation, vec2 scale, Texture* texture,
                                 float* color, bool is_hidden, Renderable* renderable)
{
    ...
    // Set opacity
    GLint is_hidden_uloc = glGetUniformLocation(renderable->effect.program, "is_hidden");
