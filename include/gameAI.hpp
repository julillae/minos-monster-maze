...
class GameAI {
    ...
    public:
        ...
        bool static canMoveHere(int row, int col);
        ...
        stack<vec2> static getPath(std::vector<std::vector<tile>> tileDetails, TileCoord src, TileCoord dest);

        bool static compare(cTileCoord left, cTileCoord right);
        ...
        void static processChildAStar(TileCoord parent, TileCoord child, TileCoord dest, priority_queue<cTileCoord, std::vector<cTileCoord>, std::function<bool(cTileCoord, cTileCoord)>> &frontier, std::vector<std::vector<tile>> &tileDetails);

        stack<vec2> static followPlayer(vec2 source, SearchMethod method);
